...
TOKEN = 5855114839:AAGpvV3iq8ZUHLTlkwcQLV_GUA1Xa2iQ-Lk 
...
def send_welcome(message): 
    ...
    link2 = int(message.text.split(' ')[2]) + 1 
    s = requests.session() 
    ...
    for i in range(1, link2): 
        bot.send_message(message.chat.id, 
                         r.text.split('img class="serp-item__thumb justifier__thumb"')[i].split('data-error-handler=')[ 
                             0]) 
